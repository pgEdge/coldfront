...
-- _detach_partition_peers: run the SAME `DETACH PARTITION … CONCURRENTLY` on
-- every OTHER mesh node. Spock's DDL replication captures DDL inside the
-- replicating transaction, but `DETACH … CONCURRENTLY` cannot run in a
-- transaction block (Postgres splits it across two), so Spock silently skips it
-- ("WARNING: This DDL statement will not be replicated") — leaving the partition
-- attached on peers while the origin detaches it. The partition manager's
-- retention path (internal/partition Manager.Detach) detaches concurrently to
-- avoid an ACCESS EXCLUSIVE lock on the parent; to stay non-blocking AND keep
-- the mesh consistent, it runs the local detach top-level, then calls this to
-- fan the identical concurrent detach to each peer. Each peer runs it in its
-- OWN autocommit session (dblink_exec sends one top-level command), so it is not
-- inside a transaction block and succeeds. Idempotent: a peer where the
-- partition is already detached (or gone) is skipped, so a re-run after a
-- partial fan-out neither errors nor double-detaches. No-op without spock
-- (vanilla single node has no peers). NB the archiver's COLD cutover detach
-- (coldfront.cutover_archive) is a PLAIN, transactional DETACH and so replicates
-- on its own — only the concurrent retention detach needs this.
CREATE OR REPLACE FUNCTION coldfront._detach_partition_peers(
    p_parent text, p_partition text
...
BEGIN
    ...
    END IF;
    FOR r IN
        SELECT nd.node_name AS name, ni.if_dsn AS dsn
          FROM spock.node nd
          JOIN spock.node_interface ni ON ni.if_nodeid = nd.node_id
         WHERE nd.node_id <> (SELECT node_id FROM spock.local_node)
    LOOP
        cn := 'coldfront_detach_' || r.name;
        IF cn = ANY (COALESCE(public.dblink_get_connections(), '{}'::text[])) THEN
            PERFORM public.dblink_disconnect(cn);
        END IF;
        PERFORM public.dblink_connect(cn, r.dsn);
        -- to_regclass (not ::regclass) so a peer that already dropped/detached the
        -- partition yields NULL → 0 rows → skip, instead of raising.
        SELECT t.c::int INTO attached FROM public.dblink(cn, format(
            'SELECT count(*) FROM pg_inherits WHERE inhparent = to_regclass(%L) AND inhrelid = to_regclass(%L)',
            p_parent, p_partition)) AS t(c text);
        IF attached > 0 THEN
            PERFORM public.dblink_exec(cn, format(
                'ALTER TABLE %s DETACH PARTITION %s CONCURRENTLY', p_parent, p_partition));
            n := n + 1;
        END IF;
        PERFORM public.dblink_disconnect(cn);
    END LOOP;
