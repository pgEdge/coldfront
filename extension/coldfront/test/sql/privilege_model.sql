...
--
-- Four invariants let cold I/O work for a NON-superuser app role that holds only
...
--       so it is NOT executable by PUBLIC; the bakery reaches it only from
--       SECURITY DEFINER code and the replica-only apply trigger.
...
--     functions (read spock.* / pg_stat_replication + loop back the claim, so a
...
-- (4) PUBLIC cannot execute the loopback.
SELECT has_function_privilege('cf_priv_check',
                              'coldfront._loopback(text)'::regprocedure,
                              'execute') AS app_can_use_loopback;
