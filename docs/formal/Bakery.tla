...
(* both commit, racing the CAS. Bakery_samenode_race.cfg proves that     *)
(* violates NoLakekeeperConflict; Bakery_samenode.cfg proves the lock    *)
...
(* the CAS parent UNDER the claim.  Bakery.cfg (AsyncParquet = FALSE) is    *)
(* its safety proof; Bakery_race.cfg proves the patchless-async shortcut    *)
...
CONSTANTS Writers,
          NodeParts,      \* partition of Writers into nodes -- each element is the
                          ...
                          \* concurrent cold sessions). All-singletons is one
                          \* writer per node.
          MaxTickets,
          MaxIcebergLen,
          MaxCrashes,
...
define
  ...
  \* split below can.  Expected to be VIOLATED in Bakery_live.cfg (the bug),
