# pgEdge MultiTier vs the lakehouse-Postgres incumbents

## TL;DR — KISS wins

MultiTier is **stock upstream PostgreSQL 17/18 + two extensions + one
...
| | **MultiTier** | **EDB PGAA** | **Databricks Lakebase** | **Snowflake PG / `pg_lake`** |
|---|---|---|---|---|
...
| PG extensions required | **2** (`pg_duckdb`, `multitier`) | `pgaa` TAM + PGD stack | Lakebase runtime + `pg_mooncake` + `pg_moonlink` | **15+** (`pg_lake_iceberg`, `pg_lake_table`, `pg_lake_copy`, `duckdb_pglake`, …) |
...
MultiTier is the only option that combines transparent tiering with
...
| | **MultiTier** | **EDB PGAA** | **Lakebase** | **Snowflake PG / `pg_lake`** |
|---|---|---|---|---|
...
MultiTier's hot tier is indistinguishable from a plain partitioned
...
| Cross-tier `UPDATE`/`DELETE` | Yes in permissive mode; opt-out via `multitier.allow_mixed_writes = off` | **No** | N/A | No — two tables, two statements |
...
MultiTier is the only option where cold data is mutable under the
...
`multitier.allow_mixed_writes = off` — ambiguous cross-tier writes
are then rejected with a clear error.  The writable-cold model is
opt-in, not imposed.

## Cold-query path

MultiTier is the only option that keeps the cold query path
...
| Step | **MultiTier** | **EDB PGAA** | **Snowflake PG / `pg_lake`** |
|---|---|---|---|
| 1. PG receives query | — | — | — |
...
4. One-time arming after Lakekeeper bootstrap: an operator must
   call `SELECT multitier.arm_login_attach()` once per database for
